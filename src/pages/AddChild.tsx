...
const AddChildContent = () => {
  ...
  const isStepValid = (step: number) => {
    if (step === 0) return form.name.trim() && form.gender && form.age;
    return true; // Steps 1 and 2 are optional
  };

  const isValid = form.name.trim() && form.age && form.gender;
          ...
          <CardContent className="p-5">
            <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {sports.map((s) => (
                        <button
                          ...
                          className={cn(
                            "px-3 py-1.5 rounded-full text-xs font-medium border transition-all press-effect",
              ...
              <div className="flex gap-2 pt-2">
                {currentStep > 0 && (
                  <Button type="button" variant="outline" onClick={() => setCurrentStep(currentStep - 1)} className="flex-1 rounded-xl">
                    السابق
                  </Button>
                )}
                {currentStep < steps.length - 1 ? (
                  <Button
                    ...
                    className="flex-1 gradient-primary text-primary-foreground rounded-xl press-effect"
                  >
                    التالي
                  </Button>
                ) : (
